...
Below, the computation of the NEMO affinity graph is provided:

```{r}
AF_NEMO <- NEMO::nemo.affinity.graph(PRAD_matrix, k=n_neighbours)
```
...
```{r}
PAM_clustering_W1 <- pam_clustering(W[[1]], k)
PAM_clustering_W2 <- pam_clustering(W[[2]], k)
PAM_clustering_W3 <- pam_clustering(W[[3]], k)
PAM_clustering_W_avg <- pam_clustering(W_avg, k)
PAM_clustering_SNF <- pam_clustering(P, k)
PAM_clustering_AF_NEMO <- pam_clustering(AF_NEMO, k)
```
...
```{r}
nemo_clustering <- NEMO::nemo.clustering(PRAD_matrix, num.clusters=k, num.neighbors=n_neighbours)
```

## Spectral clustering (OPTIONAL)

We will perform *Spectral Clustering* on the integrated matrix obtained from Similarity Network Fusion, which is accessible by `SNFtool::spectralClustering()`.

```{r}
spectral_clustering <- SNFtool::spectralClustering(AF_NEMO, k)
names(spectral_clustering) <- colnames(AF_NEMO)
```
...
In the literature, several evaluation metrics can be found to compare the clustering results @wagner2007comparing, and using `mclustcomp` R package we can access to 24 different scores. Here, we will consider 3 of the most used techniques, which are descriped in the following:
...
metrics.PAM_clustering_W1 <- mclustcomp(PAM_clustering_W1$clustering, labels, types=types)
metrics.PAM_clustering_W2 <- mclustcomp(PAM_clustering_W2$clustering, labels, types=types)
metrics.PAM_clustering_W3 <- mclustcomp(PAM_clustering_W3$clustering, labels, types=types)
metrics.PAM_clustering_W_avg <- mclustcomp(PAM_clustering_W_avg$clustering, labels, types=types)
metrics.PAM_clustering_SNF <- mclustcomp(PAM_clustering_SNF$clustering, labels, types=types)
# metrics.PAM_clustering_AF_NEMO <- mclustcomp(PAM_clustering_AF_NEMO$clustering, labels, types=types)
...
clusterings_df_list <- list(
  metrics.PAM_clustering_W1,
  metrics.PAM_clustering_W2,
  metrics.PAM_clustering_W3,
  metrics.PAM_clustering_W_avg,
  metrics.PAM_clustering_SNF
)

# Extract scores column as list for each clustering
metrics_scores_lists <- lapply(clusterings_df_list, function(df) as.list(df$scores))

# Combine the lists into a single dataframe
results_df <- do.call(rbind, metrics_scores_lists)

# Correct the ordering of columns as expected
colnames(results_df) <- as.list(metrics.PAM_clustering_W1$types)
...
It is evident that the RI metric provides more optimistic results, so it would be a valid idea to present other measures such as ARI (based on counting pairs) and NMI (based on information theory) to better interpret the results. Analyzing the results of clustering algorithms comparing to PAM50, the following observations can be made:
...
-   Due to the simplicity of the chosen preprocessing techniques (such as selecting the first 100 highest variance), it is probable that some of the aforementioned group of genes are removed. This could potentially complicate the identification of subgroups based on alternative features.
